...
void print_help()
{
	...
	std::cerr << "[ display the image information ]" << std::endl;
	std::cerr << "example: ImageTools --information --in inputImage\n" << std::endl;		
	...
	std::cerr << "[ downsample image x times]" << std::endl;
	std::cerr << "example: ImageTools --downsample_xtimes --in inputImage --ratio value --out outputImage\n" << std::endl;
	...
	std::cerr << "[ crop volumn and save the padding point]" << std::endl;
	std::cerr << "example: ImageTools --crop_save_point --in inputImage --start_pt filename --end_pt filename --pad value(mm) --pad_start_pt filename --pad_end_pt filename --out outputImage\n" << std::endl;
	...
	std::cerr << "[ myocardium label transform ]" << std::endl;
	std::cerr << "example: ImageTools --myocardium_label_transform --in inputImage --out outputImage\n" << std::endl;

	std::cerr << "[ extract myocardium label ]" << std::endl;
	std::cerr << "example: ImageTools --myocardium_label_extract --in inputImage --gray value --out outputImage\n" << std::endl;
	...
	std::cerr << "[ probability map fusion ]" << std::endl;
	std::cerr << "example: ImageTools --fuse_probmap --inputPath inputImagePath --out outputImage \n" << std::endl;

	std::cerr << "[ fuse two label together ]" << std::endl;
	std::cerr << "example: ImageTools --fuse_label --in inputLabelImage --in2 inputLabelImage2 --out outputImage \n" << std::endl;

	std::cerr << "[ compute the distance map based on one landmark]" << std::endl;
	std::cerr << "example: ImageTools --distance_map --in inputImage --landmark pointfile --out outputImage \n" << std::endl;
...
///////////////////////////////////////////////////////////////////////////////
//Goal: display the image information
//Step: 1. load the reference image
//		2. display the information, including origin, spacing, dimensions, direction.
///////////////////////////////////////////////////////////////////////////////
template<typename T>
bool Information(const std::string inputImageFile) {
	...
	const char* imagefile = inputImageFile.c_str();
	if (!imagefile) {
	...
	try {
		...
		std::cerr << "ExceptionObject caught in reader!" << std::endl;
	...
	//Step 2: display the spacing, origin, dimension, direction.
	...
	const typename ImageType::RegionType scale = image->GetLargestPossibleRegion().GetSize();

	//  std::cout << "Scale = ";
	std::cout << scale << std::endl;

	const typename ImageType::DirectionType& direct = image->GetDirection();
...
bool Nearest_UpSample(const std::string referenceImage, const std::string inputImageFile, const std::string outputImageFile, double spacing_value) {
	...
	try {
		...
		std::cerr << "ExceptionObject caught in reader_refer!" << std::endl;
		std::cerr << err << std::endl;
	...
	try {
		...
		std::cerr << "ExceptionObject caught in reader_candidate!" << std::endl;
		std::cerr << err << std::endl;
	...
	try {
		...
		std::cerr << "ExceptionObject caught in writer!" << std::endl;
		std::cerr << err << std::endl;
...
///////////////////////////////////////////////////////////////////////////////
//Goal: downsample the ct heart data 
//Step: 1. load the image 
//      2. compute the output image size
//      3. distribute the parameters for resample filter
//      4. save the resampled image
///////////////////////////////////////////////////////////////////////////////
template <typename T>
bool DownSampleXTimes(const std::string input_path, double ratio, const std::string output_path) {
	...
	typename ImageType::SizeType resampleSize;
	for (int i = 0; i < 3; ++i) {
		resampleSize[i] = static_cast<unsigned int>( size[i] / ratio + 0.5 );
	}
	std::cout << "Output size: " << resampleSize << std::endl;
	std::cout << "Resampled Spacing = " << spacing[0] << ", " << spacing[1] << ", " << spacing[2] << std::endl;
	...
	//Step 4. save the resampled image to argv[2]
	typename ImageType::Pointer outputImage = resampleFilter->GetOutput();
	std::cout << "Output size: " << outputImage->GetLargestPossibleRegion().GetSize() << std::endl;

	typedef itk::ImageFileWriter<ImageType> WriteType;
	typename WriteType::Pointer writer = WriteType::New();
...
///////////////////////////////////////////////////////////////////////////////
bool GenegrateMaskImage(const std::string input_file, const double threshold, const std::string output_file) {
...
bool Voxel2World(const std::string input_path, const std::string landmark_path) {
	...
	if (point[0] < 0 || point[1] < 0 || point[2] < 0 ||
		static_cast<unsigned int>(point[0]) > image->GetLargestPossibleRegion().GetSize()[0] ||
		static_cast<unsigned int>(point[1]) > image->GetLargestPossibleRegion().GetSize()[1] ||
		static_cast<unsigned int>(point[2]) > image->GetLargestPossibleRegion().GetSize()[2]) {
		isInside = 0;
	}
	else {
		isInside = 1;
...
bool Crop(const std::string input_path, const std::string start, const std::string end, const int pad, const std::string output_path) {
	...
	int padpixel[3];
	for (int i = 0; i < 3; i++) {
		padpixel[i] = static_cast<int>(pad / spacing[i] + 0.5);
	}

	std::cout << "pad pixel = " << padpixel[0] << ", " << padpixel[1] << ", " << padpixel[2] << std::endl;

	//revise the bounding box size
	for (int i = 0; i < 3; i++) {
		int temp = startIndex[i] - padpixel[i];
		if (temp < 0) {
			startIndex[i] = 0;
		}
		else
	...
	for (int i = 0; i < 3; i++) {
		int temp2 = endIndex[i] + padpixel[i];
...
//		3. output the image to file
///////////////////////////////////////////////////////////////////////////////
bool CropSavePoint(const std::string input_path, const std::string start, const std::string end, 
	const int pad, const std::string pad_start, const std::string pad_end, const std::string output_path) {
	...
	ImageType::SizeType desiredSize;
	for (int i = 0; i < 3; i++) {
		desiredSize[i] = endIndex[i] - startIndex[i] + 1;
	}
	
	ImageType::RegionType desiredRegion(startIndex, desiredSize);

	std::cout << "desiredRegion: " << desiredRegion << std::endl;
	std::cout << "start index = [" << startIndex[0] << ", " << startIndex[1] << ", " << startIndex[2] << "]" << std::endl;
	std::cout << "end index = [" << endIndex[0] << ", " << endIndex[1] << ", " << endIndex[2] << "]" << std::endl;
	...
	//4. output the two padding points world coordinate to txt file
	PointType world_start;
	image->TransformIndexToPhysicalPoint(startIndex, world_start);
	std::cout << "Start point's world coordinate = " << world_start[0] << ", " << world_start[1] << ", " << world_start[2] << std::endl;

	PointType world_end;
	image->TransformIndexToPhysicalPoint(endIndex, world_end);
	std::cout << "End point's world coordinate = " << world_end[0] << ", " << world_end[1] << ", " << world_end[2] << std::endl;

	//start point 
	std::ofstream start_file(pad_start, std::ios::out);
	if (!start_file)
	{
		std::cout << "Error opening file for writing." << std::endl;
		return EXIT_FAILURE;
	...
	//end point
	std::ofstream end_file(pad_end, std::ios::out);
	if (!end_file)
	{
		std::cout << "Error opening file for writing." << std::endl;
		return EXIT_FAILURE;
...
bool PutBack(const std::string ref_path, const std::string input_path, const std::string start_pt, const std::string end_pt, const std::string output_path) {
	...
	try {
		...
		std::cerr << "ExceptionObject caught in in_reader!" << std::endl;
		std::cerr << err << std::endl;
...
///////////////////////////////////////////////////////////////////////////////
//Goal: Myocardium label transform
//Step: 1. Read the CT data image
//      2. Change the old label value to 0-7.
//		3. Save the CT data image to NiFTI type data 
///////////////////////////////////////////////////////////////////////////////
bool MyocardiumLabelTransform(const std::string input_path, const std::string output_path) {
	typedef unsigned short						PixelType;
	const unsigned int                  Dimension = 3;
...
///////////////////////////////////////////////////////////////////////////////
//Goal: Myocardium label extract
//Step: 1. Read the CT data image
//      2. Extract the myocardium region
//		3. Save the CT data image to NiFTI type data 
///////////////////////////////////////////////////////////////////////////////
bool MyocardiumLabelExtract(const std::string input_path, const int gray, const std::string output_path) {
	typedef unsigned short						PixelType;
	const unsigned int                  		Dimension = 3;
...
bool Majority_Vote(const std::string inputPath, const std::string output_path) {
	...
	const char* filelist = inputPath.c_str();

	std::ifstream infile;
	infile.open(filelist, std::ifstream::in);

	if (!infile) {
		std::cerr << "Load input filelist error!" << std::endl;
		return EXIT_FAILURE;
	}
...
///////////////////////////////////////////////////////////////////////////////
//Goal: fuse the probability map
//Step: 1. read the input probability maps 
//      2. fuse the probability map
//		3. output the fused probmap image to file
///////////////////////////////////////////////////////////////////////////////
bool Fuse_Porbmap(const std::string inputPath, const std::string output_path) {

	//1. read the input probability maps 
	typedef double										PixelType;
	...
	for (outIter.GoToBegin(); !outIter.IsAtEnd(); ++outIter) {
		...
		for (int j = 0; j < images.size(); j++) {
			...
			fused_value += tmpIter.Get();

		}

		fused_value /= static_cast<double>( images.size() );

		outIter.Set( fused_value );
...
///////////////////////////////////////////////////////////////////////////////
//Goal: fuse two label image
//Step: 1. read the two input label image
//      2. fuse the label image
//		3. output the fused label image to file
///////////////////////////////////////////////////////////////////////////////
bool Fuse_Label(const std::string input_path, const std::string input_path2, const std::string output_path) {
	
	//1. read the two input label image
	std::cout << "Fuse_Label  		START" << std::endl;
	...
	const char* filename2 = input_path2.c_str();
	if (!filename2) {
		std::cout << "Load input file error!" << std::endl;
		return EXIT_FAILURE;
	}
	...
	try {
		...
		std::cerr << "ExceptionObject caught in reader2!" << std::endl;
		std::cerr << err << std::endl;

		return EXIT_FAILURE;
	}

	std::cout << "Read image2 success!" << std::endl;
	...
	IteratorType outIter( image, image->GetRequestedRegion() );
	ConstIteratorType tmpIter( image2, image2->GetRequestedRegion() );

	for (outIter.GoToBegin(), tmpIter.GoToBegin(); !outIter.IsAtEnd(); ++outIter, ++tmpIter) {

		if ( tmpIter.Get() == 1 || outIter.Get() == 1) {
			outIter.Set( 1 );
		}
...
///////////////////////////////////////////////////////////////////////////////
//Goal: compute the distance map based on one landmark
//Step: 1. load the input image and landmark position
//      2. convert the landmark position from world coordinate to voxel index coordiante
//		3. compute the distance map
//		4. output the distance map to file
///////////////////////////////////////////////////////////////////////////////
bool Distance_Map(const std::string input_path, const std::string landmark_path, const std::string output_path) {
	
	//1. load the input image and landmark position
	typedef double										PixelType;
	const unsigned int Dimension = 3;
	...
	ImageType::IndexType tmpIdx;
	int count = 0;
	for (outIter.GoToBegin(); !outIter.IsAtEnd(); ++outIter) {

		tmpIdx = outIter.GetIndex();
		
		// if (count < 5) {
			
		// 	std::cout << "tmpIdx's voxel coordinate = " << 
		// 		tmpIdx[0] << ", " << tmpIdx[1] << ", " << tmpIdx[2] << std::endl;
			
		// 	count++;
		// }

		double dm = static_cast<double>( 
			 (tmpIdx[0]-pixelIndex[0])*(tmpIdx[0]-pixelIndex[0]) +
			 (tmpIdx[1]-pixelIndex[1])*(tmpIdx[1]-pixelIndex[1]) +
			 (tmpIdx[2]-pixelIndex[2])*(tmpIdx[2]-pixelIndex[2]) 
			 );
	...
	//3. output the distance map to file
	typedef itk::ImageFileWriter<ImageType> 		 WriteType;
	WriteType::Pointer writer = WriteType::New();
	...
	std::cout << "Write output image to file success!" << std::endl;
...
int main(int argc, char * argv[])
{
	try {
		...
		functional.add_options()("information", "display the image information");
		...
		functional.add_options()("downsample_xtimes", "downsample an image x times");
		...
		functional.add_options()("crop_save_point", "crop volumn and save the padding point");
		...
		functional.add_options()("myocardium_label_transform", "image label transform");
		functional.add_options()("myocardium_label_extract", "myocardium label extract");
		...
		functional.add_options()("fuse_probmap", "probability map fusion");
		functional.add_options()("fuse_label", "fuse two label together");
		functional.add_options()("distance_map", "compute the distance map based on one landmark");


		options_description params("Parameter options");
		params.add_options()("in", value<std::string>(), "input image");
		params.add_options()("in2", value<std::string>(), "input image 2");
		...
		params.add_options()("pad_start_pt", value<std::string>(), "padding start point file");
		params.add_options()("pad_end_pt", value<std::string>(), "padding end point file");
		params.add_options()("gray", value<int>(), "gray value");
		...
		if (vm.count("information")) {
			...
			switch (type)
			{
			case _UCHAR: Information<unsigned char>(input_path);  break;
			case _CHAR: Information<char>(input_path);  break;
			case _USHORT: Information<unsigned short>(input_path);  break;
			case _SHORT: Information<short>(input_path);  break;
			case _UINT: Information<unsigned int>(input_path);  break;
			case _INT: Information<int>(input_path);  break;
			case _ULONG: Information<unsigned long>(input_path);  break;
			case _LONG: Information<long>(input_path);  break;
			case _FLOAT: Information<float>(input_path);  break;
			case _DOUBLE: Information<double>(input_path);  break;
			default: std::cout << "unrecognized pixel type";  break;
			}

		}
		else if (vm.count("nearest_upsample")) // nearest upsample
		{
			std::string input_path = Get<std::string>(vm, "in");
		...
		else if (vm.count("downsample_xtimes")) //downsample
		{
			std::string input_path = Get<std::string>(vm, "in");
			...
			switch (type)
			{
			case _UCHAR: DownSampleXTimes<unsigned char>(input_path, ratio, output_path);  break;
			case _CHAR: DownSampleXTimes<char>(input_path, ratio, output_path);  break;
			case _USHORT: DownSampleXTimes<unsigned short>(input_path, ratio, output_path);  break;
			case _SHORT: DownSampleXTimes<short>(input_path, ratio, output_path);  break;
			case _UINT: DownSampleXTimes<unsigned int>(input_path, ratio, output_path);  break;
			case _INT: DownSampleXTimes<int>(input_path, ratio, output_path);  break;
			case _ULONG: DownSampleXTimes<unsigned long>(input_path, ratio, output_path);  break;
			case _LONG: DownSampleXTimes<long>(input_path, ratio, output_path);  break;
			case _FLOAT: DownSampleXTimes<float>(input_path, ratio, output_path);  break;
			case _DOUBLE: DownSampleXTimes<double>(input_path, ratio, output_path);  break;
			default: std::cout << "unrecognized pixel type"; break;
			}
		...
		else if (vm.count("crop_save_point")) //crop
		{
			std::string input_path = Get<std::string>(vm, "in");
			std::string output_path = Get<std::string>(vm, "out");
			std::string start_pt = Get<std::string>(vm, "start_pt");
			std::string end_pt = Get<std::string>(vm, "end_pt");
			std::string pad_start_pt = Get<std::string>(vm, "pad_start_pt");
			std::string pad_end_pt = Get<std::string>(vm, "pad_end_pt");
			int pad = Get<int>(vm, "pad");
			...
			std::cout << "end_pt = " << end_pt << std::endl;
			std::cout << "pad = " << pad << std::endl;
			std::cout << "pad_start_pt = " << pad_start_pt << std::endl;
			std::cout << "pad_end_pt = " << pad_end_pt << std::endl;

			CropSavePoint(input_path, start_pt, end_pt, pad, pad_start_pt, pad_end_pt, output_path);
		...
		else if (vm.count("myocardium_label_transform")) //myocardium_label_transform
		{
			std::string input_path = Get<std::string>(vm, "in");
			std::string output_path = Get<std::string>(vm, "out");
			...
			MyocardiumLabelTransform(input_path, output_path);
		}
		else if (vm.count("myocardium_label_extract")) //myocardium_label_extract
		{
			std::string input_path = Get<std::string>(vm, "in");
			std::string output_path = Get<std::string>(vm, "out");
			int gray = Get<int>(vm, "gray");
			
			std::cout << "input_path = " << input_path << std::endl;
			std::cout << "output_path = " << output_path << std::endl;
			std::cout << "gray = " << gray << std::endl;

			MyocardiumLabelExtract(input_path, gray, output_path);
		}
		...
		else if (vm.count("fuse_probmap")) //fuse_probmap
		{
			std::string inputPath = Get<std::string>(vm, "inputPath");
			std::string output_path = Get<std::string>(vm, "out");
			...
			Fuse_Porbmap(inputPath, output_path);
		}
		else if (vm.count("fuse_label")) //fuse_label
		{
			std::string input_path = Get<std::string>(vm, "in");
			std::string input_path2 = Get<std::string>(vm, "in2");
			std::string output_path = Get<std::string>(vm, "out");
			...
			Fuse_Label(input_path, input_path2, output_path);
		}
		else if (vm.count("distance_map")) //distance_map
		{
			std::string input_path = Get<std::string>(vm, "in");
			std::string landmark_path = Get<std::string>(vm, "landmark");
			std::string output_path = Get<std::string>(vm, "out");
			...
			Distance_Map(input_path, landmark_path, output_path);
		}
		else
		{
